...
parallel_scheduler::parallel_scheduler(size_t _thread_count)
    : thread_count(_thread_count), threads(new pthread_t[_thread_count]),
      mutex(new pthread_mutex_t(PTHREAD_MUTEX_INITIALIZER)), cond(new pthread_cond_t(PTHREAD_COND_INITIALIZER)),
      is_working(true) {
  // create threads
  for (size_t i = 0; i < thread_count; ++i) {
    error_check(pthread_create(&threads[i], nullptr, consumer, (void *) this));
  }
}
...
parallel_scheduler::~parallel_scheduler() {
  ...
  // join threads
  for (size_t i = 0; i < thread_count; ++i) {
    error_check(pthread_join(threads[i], nullptr));
  }
...
void parallel_scheduler::run(const std::function<void(void *)> &func, void *arg) {
  // lock
